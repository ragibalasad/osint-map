...
import { 
  X, MapPin, Loader2, RefreshCcw, Activity, ShieldCheck, 
  ChevronRight, Trash2, Zap, AlertTriangle, AlertCircle, ExternalLink, Brain
} from "lucide-react";
  ...
  return (
    <div className="h-screen flex flex-col bg-background font-sans overflow-hidden">
      <header className="h-16 border-b border-border/40 bg-background/50 backdrop-blur-3xl flex items-center justify-between px-8 z-20">
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
                <h1 className="text-xs font-black tracking-widest uppercase text-foreground font-display leading-none flex items-center gap-2">
                  Tactical Response Hub
                  <Badge variant="outline" className="h-4 text-[8px] font-black tracking-tighter border-primary/30 text-primary bg-primary/5 uppercase">Admin v2</Badge>
                </h1>
                <span className="text-[9px] font-black text-muted-foreground/60 uppercase tracking-[0.3em] leading-none mt-1 opacity-80">Geospatial Intelligence (MOD)</span>
            </div>
          </div>
          <div className="flex items-center gap-4">
             <div className="flex items-center gap-2 bg-secondary/30 px-3.5 py-1.5 rounded-xl border border-border/40 backdrop-blur-md">
                ...
                <span className="text-[10px] font-black text-foreground/80 lowercase tracking-tighter">{queue?.length || 0} signals active</span>
             </div>
             {/* AI Provider Toggle */}
             <div className="flex items-center gap-1 bg-secondary/20 p-1 rounded-xl border border-border/30">
              ...
              <Button 
                ...
                className="gap-2 h-9 rounded-xl px-4 text-[10px] font-black uppercase tracking-widest text-destructive/80 border-destructive/20 hover:bg-destructive/10 bg-destructive/5 hover:text-destructive transition-all"
              >
                <Trash2 className="w-3.5 h-3.5" /> Purge Queue
              </Button>
             <Button variant="outline" size="sm" onClick={() => mutate()} className="gap-2 h-9 rounded-xl px-4 text-[10px] font-black uppercase tracking-widest bg-secondary/20 hover:bg-secondary/40">
      ...
      <div className="flex-1 flex overflow-hidden">
        {/* List Section */}
        <div className="w-80 h-full border-r border-border/30 flex flex-col bg-card/10 backdrop-blur-xl">
          <div className="flex-1 overflow-y-auto custom-scrollbar p-4 space-y-6">
            {isLoading && (
              <div className="flex flex-col items-center justify-center py-20 gap-4 text-muted-foreground opacity-30">
                ...
                <span className="text-[10px] font-black uppercase tracking-[0.3em] font-display">Syncing Registry...</span>
              </div>
            )}
            ...
            {!isLoading && Object.entries(groupedQueue).map(([date, items]) => (
              <div key={date} className="space-y-4">
                <div className="sticky top-0 z-10 bg-background/60 backdrop-blur-2xl py-2 px-1 border-b border-border/20 mb-3 flex items-center justify-between">
                  <span className="text-[9px] font-black tracking-[0.2em] uppercase text-primary/80 flex items-center gap-2">
                    <Activity className="w-3 h-3 h-3" />
                    {date}
                  </span>
                  <Badge variant="outline" className="h-4 text-[8px] border-border/30 opacity-50 px-1">{items.length} MSG</Badge>
                </div>
                {items.map((item) => {
                    ...
                    >
                      <div className="flex flex-col gap-2 relative z-10">
                        <div className="flex items-start justify-between gap-2">
                          <h3 className={cn(
                            "font-black text-[10px] tracking-tight line-clamp-2 flex-1 uppercase font-display leading-[1.35]",
                            selected?.id === item.id ? "text-primary" : 
                            item.status === "processing" ? "text-primary/60 animate-pulse" :
                            item.status === "failed" ? "text-destructive/80" :
                            item.suggestedTitle ? "text-foreground" : "text-muted-foreground"
                          )}>
                            {item.status === "processing" ? "PROBING SIGNAL..." : 
                             item.status === "failed" ? "ENRICHMENT FAILED: SIGNAL DEGRADED" :
                        ...
                        <div className="flex items-center justify-between mt-2 pt-2 border-t border-border/10">
                          <div className="flex items-center gap-2">
                            <span className="text-[9px] font-black text-muted-foreground/40 tabular-nums uppercase">
                              {formatRelativeTime(item.sourceCreatedAt || item.createdAt)}
                            </span>
                            {eventLink && (
                              <Badge variant="outline" asChild className="text-[7px] h-3.5 px-1.5 font-black border-primary/30 text-primary bg-primary/5 uppercase tracking-tighter hover:bg-primary/10 cursor-pointer">
                                <a href={eventLink} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()}>
                                  {item.source}
                                </a>
                              </Badge>
                            )}
                            {!eventLink && item.source && (
                              <Badge variant="outline" className="text-[7px] h-3.5 px-1.5 font-black border-primary/30 text-primary bg-primary/5 uppercase tracking-tighter">
                                {item.source}
                              </Badge>
                            )}
                          </div>
                          <ChevronRight className={cn("w-3.5 h-3.5 transition-all text-muted-foreground/20", selected?.id === item.id ? "text-primary translate-x-1" : "group-hover:text-primary/50")} />
                        </div>
                      </div>
                    </div>
                  );
            ...
            {!isLoading && queue?.length === 0 && (
              <div className="text-center py-32 flex flex-col items-center gap-6 opacity-20">
                ...
                <p className="text-[10px] font-black uppercase tracking-[0.5em] font-display">Sector Neutral</p>
              </div>
            )}
          </div>
        </div>
        ...
        {/* Details & Map Section */}
        <div className="flex-1 h-full bg-background relative flex flex-col shadow-[inset_24px_0_48px_-24px_rgba(0,0,0,0.5)]">
          {selected ? (
            <div className="flex-1 flex flex-col md:flex-row overflow-hidden animate-in fade-in zoom-in-95 duration-500">
              <div className="flex-1 flex flex-col border-r border-border/20">
                <div className="flex-1 overflow-y-auto custom-scrollbar p-10 space-y-10">
                  <div className="space-y-8">
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center gap-3">
                         <div className={cn("w-2.5 h-2.5 rounded-full shadow-[0_0_12px_rgba(var(--primary),0.8)]", 
                           selected.status === "failed" ? "bg-destructive shadow-destructive" : "bg-primary shadow-primary"
                         )} />
                         <h2 className="text-[10px] font-black uppercase tracking-[0.4em] text-foreground/70 font-display">
                           {selected.status === "failed" ? "Enrichment Failure" : "Enrichment Protocol"}
                         </h2>
                      </div>
                      {selected.status === "failed" && (
                        <Badge variant="destructive" className="animate-pulse text-[8px] font-black uppercase tracking-widest">Permanent Loss</Badge>
                      )}
                    </div>
                    ...
                    {selected.status === "failed" && (
                      <div className="bg-destructive/10 border border-destructive/20 rounded-2xl p-4 flex items-center gap-4 text-destructive mb-6">
                        ...
                        <div className="flex flex-col gap-0.5">
                          <span className="text-[10px] font-black uppercase tracking-widest leading-none">AI Parsing Blocked</span>
                          <span className="text-[10px] opacity-70 font-medium tracking-tight">System was unable to enrich this signal after the initial attempt (Quota/Rate Limit). Manual entry required.</span>
                        </div>
                      </div>
                    )}
                    
                    <div className="space-y-2">
                      <label className="text-[10px] font-black uppercase text-muted-foreground/60 tracking-[0.2em] ml-1">Assigned Title</label>
                      <input 
                        className="w-full bg-secondary/20 border border-border/30 rounded-2xl p-5 text-sm font-black focus:outline-none focus:ring-2 focus:ring-primary/40 focus:bg-background transition-all font-display ring-offset-background selection:bg-primary/30"
                        value={editTitle}
                    ...
                    <div className="space-y-2">
                      <label className="text-[10px] font-black uppercase text-muted-foreground/60 tracking-[0.2em] ml-1">Summary SITREP</label>
                      <textarea 
                        className="w-full bg-secondary/20 border border-border/30 rounded-2xl p-5 text-sm h-52 resize-none focus:outline-none focus:ring-2 focus:ring-primary/40 focus:bg-background transition-all leading-relaxed font-medium selection:bg-primary/30"
                        value={editDesc}
                    ...
                    <div className="bg-secondary/10 border border-border/30 rounded-3xl p-8 space-y-4 relative overflow-hidden group">
                      ...
                      <div className="flex justify-between items-center pb-3 border-b border-border/20">
                        <label className="text-[10px] font-black uppercase text-muted-foreground/80 tracking-widest">Signal Source Content</label>
                        <div className="flex items-center gap-2">
                          {extractUrl(selected) && (
                            <a 
                              ...
                              className="flex items-center gap-1 text-[10px] font-black border border-primary/20 text-primary bg-primary/5 hover:bg-primary/10 px-2 py-0.5 rounded-full transition-all"
                            >
                              <ExternalLink className="w-3 h-3" /> Open Source
                            </a>
                          )}
                          <Badge variant="outline" className="text-[10px] font-black border-emerald-500/20 text-emerald-500 bg-emerald-500/5 px-2">ENCRYPTED</Badge>
                        </div>
                      </div>
                ...
                <div className="h-28 border-t border-border/20 bg-background/50 backdrop-blur-2xl p-8 flex flex-row-reverse items-center justify-between gap-6">
                  <div className="flex gap-3">
                    <Button 
                      variant="outline"
                      className="h-12 px-6 rounded-2xl text-[10px] font-black uppercase tracking-widest gap-3 border-border/40 hover:bg-primary/10 hover:border-primary/40 transition-all text-foreground group"
                      onClick={handleReprocess}
                      disabled={isReprocessing || isPublishing}
                    >
                      {isReprocessing ? <Loader2 className="animate-spin w-4 h-4" /> : <Zap className="w-4 h-4 group-hover:fill-primary" />}
                      Rescan
                    </Button>
                    <Button 
                      variant="ghost" 
                      className="h-12 w-12 rounded-2xl text-muted-foreground/40 hover:bg-destructive/10 hover:text-destructive border border-transparent hover:border-destructive/20 transition-all"
                    ...
                    <Button 
                      className="h-12 px-10 rounded-2xl text-[10px] font-black uppercase tracking-widest gap-4 shadow-2xl shadow-primary/30 hover:shadow-primary/50 hover:scale-[1.02] active:scale-[0.98] transition-all bg-primary hover:bg-primary/90 text-primary-foreground"
                      onClick={handlePublish}
                      disabled={isPublishing}
                    >
                      {isPublishing ? <Loader2 className="animate-spin" /> : <ShieldCheck className="w-4 h-4 stroke-[3]" />}
                      Authorize Deployment
                    </Button>
                  </div>
                  <div className="flex flex-col items-start gap-1 p-2 px-4 rounded-xl border border-border/10 bg-secondary/5">
                     <span className="text-[9px] font-black uppercase tracking-[0.3em] text-muted-foreground/40">Intel ID Hash</span>
                     <span className="text-[10px] font-mono text-muted-foreground/60">{selected.id.split('-')[0]}...{selected.id.split('-').pop()}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="w-[450px] h-full flex flex-col bg-card/5">
                <div className="p-8 border-b border-border/20 bg-background/30 backdrop-blur-md space-y-4">
                    ...
                    <div className="flex items-center justify-between">
                       {editPos ? (
                         ...
                         <span className="text-[10px] font-mono text-destructive font-black tracking-tight flex items-center gap-2 italic animate-pulse">
                           <AlertTriangle className="w-3 h-3" />
                           NO_COORDINATES_FOUND
                         </span>
                       )}
                       <span className="text-[10px] font-mono text-foreground font-black tracking-tighter bg-secondary/30 px-2 py-0.5 rounded border border-border/20">
                          {editPos ? `${editPos.lat.toFixed(6)}°N / ${editPos.lng.toFixed(6)}°E` : "UNKNOWN_LOC"}
                       </span>
                    </div>
                </div>
                <div className="flex-1 relative">
                    ...
                    <div className="absolute top-6 left-6 right-6 z-10 flex flex-col gap-3">
                       <Card className="bg-background/80 backdrop-blur-2xl border-primary/20 p-3 text-center text-[10px] font-black tracking-[0.3em] uppercase shadow-[0_12px_48px_-12px_rgba(0,0,0,0.8)] border-dashed border-2">
                       ...
                       {selected?.lng && (
                         <Button 
                           ...
                           className="bg-primary/10 backdrop-blur-2xl border-primary/40 h-10 text-[10px] font-black uppercase tracking-widest gap-3 hover:bg-primary/20 transition-all text-primary shadow-2xl"
                         >
            ...
            <div className="h-full flex flex-col items-center justify-center text-center p-20 animate-in fade-in zoom-in-95 duration-1000">
              <div className="relative mb-12">
                ...
                <div className="w-32 h-32 rounded-[2.5rem] bg-secondary/20 flex items-center justify-center border border-primary/10 relative overflow-hidden backdrop-blur-2xl shadow-inner">
                   ...
                   <div className="absolute bottom-1 w-full text-center">
                      <div className="text-[7px] font-black text-primary/30 tracking-[0.5em] uppercase">Status_Idle</div>
                   </div>
                </div>
              </div>
              ...
              <p className="text-sm text-muted-foreground/60 max-w-lg antialiased leading-relaxed font-medium uppercase tracking-tight px-10 border-l border-r border-border/20">
                Awaiting incoming raw signals. Initiate MTProto client extraction or select a pending tactical event from the registry to begin enrichment protocol.
              </p>
            </div>
          )}
        </div>
        ...
        <DialogContent className="max-w-md bg-background/95 backdrop-blur-2xl border-destructive/20 rounded-3xl p-8 font-sans">
          <DialogHeader className="space-y-4 text-center">
            ...
            <DialogDescription className="text-muted-foreground/60 font-medium leading-relaxed">
              EXTREME CAUTION: You are about to initiate a terminal wipe of the entire moderation queue. This action will permanently erase all unverified intelligence reports. Data recovery is not possible.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter className="flex-col sm:flex-row gap-3 pt-6 border-t border-border/20 mt-6">
            <Button variant="ghost" onClick={() => setShowClearDialog(false)} className="flex-1 h-12 rounded-xl text-[10px] font-black uppercase tracking-widest">
              Abort Purge
            </Button>
            <Button 
              variant="destructive" 
              ...
              disabled={isPublishing}
              className="flex-1 h-12 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-xl shadow-destructive/20"
            >
