...
export function AdminSidebar() {
  ...
  if (!mounted) return <aside className="fixed left-0 top-0 h-screen w-64 bg-background border-r border-border/50 z-50" />;

  const isVisuallyExpanded = mode === "expanded" || (mode === "hover" && isHovering);
  ...
  return (
    ...
    >
      {/* Header */}
      <div className={cn("flex items-center px-4 h-16 border-b border-border/20 shrink-0", 
        ...
        {(isVisuallyExpanded) && (
          <Link href="/" className="flex items-center gap-2 group overflow-hidden">
            ...
            <span className="font-bold text-xl tracking-tight uppercase text-foreground font-display transition-opacity duration-300 whitespace-nowrap">Hub</span>
          </Link>
        )}
      ...
      {/* Nav Items */}
      <nav className={cn("flex-1 py-6 space-y-1.5", isVisuallyExpanded ? "px-4" : "px-2")}>
        {isVisuallyExpanded && (
          ...
          return (
            <Link
              ...
              className={cn(
                ...
                isVisuallyExpanded ? "justify-between px-4 py-3" : "justify-center p-3",
                isActive 
                  ? "bg-primary/10 text-primary border border-primary/20" 
                  : "text-muted-foreground hover:bg-secondary/50 hover:text-foreground"
              )}
            >
              <div className={cn("flex items-center", isVisuallyExpanded ? "gap-3" : "")}>
                ...
                {isVisuallyExpanded && <span className="opacity-100 transition-opacity duration-300 whitespace-nowrap">{item.title}</span>}
              </div>
      ...
      <div className={cn("mt-auto", isVisuallyExpanded ? "p-4" : "p-2")}>
              ...
              {isVisuallyExpanded ? (
                <div className="bg-secondary/20 rounded-2xl p-4 border border-border/10 hover:bg-secondary/30 transition-all flex items-center gap-3">
                  ...
                  <div className="flex flex-col min-w-0">
                    <span className="text-sm font-black truncate text-foreground/90">{session?.user.name}</span>
                    <span className="text-[9px] text-primary font-black uppercase tracking-widest opacity-80">System Admin</span>
                  </div>
                </div>
              ) : (
                <div className="flex flex-col items-center pb-4 cursor-pointer">
                  <div className={cn("w-10 h-10 rounded-full bg-primary/20 flex items-center justify-center border border-primary/20 overflow-hidden transition-all group-hover/user:scale-110", isHovering && "scale-110")}>
                    {session?.user.image ? (
          ...
          <DropdownMenuContent side="right" align="end" sideOffset={16} className="w-56">
            <DropdownMenuLabel className="font-sans">
              <div className="flex flex-col gap-0.5">
                ...
                <span className="text-[10px] text-muted-foreground font-medium uppercase tracking-tighter">System Administrator</span>
              </div>
            </DropdownMenuLabel>
            <DropdownMenuSeparator />
        ...
        <div className={cn("flex items-center border-t border-border/10 pt-4 mt-2", isVisuallyExpanded ? "justify-between px-4" : "justify-center")}>
          {isVisuallyExpanded && (
            <span className="text-[9px] font-black text-muted-foreground/30 uppercase tracking-[0.2em]">v0.1-α</span>
          )}
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align={isVisuallyExpanded ? "end" : "center"} side="right" sideOffset={16} className="w-48">
              <DropdownMenuLabel>Sidebar control</DropdownMenuLabel>
              <DropdownMenuSeparator />
              <DropdownMenuItem onClick={() => changeMode("expanded")} className="flex items-center justify-between">
                Expanded
                {mode === "expanded" && <div className="w-1.5 h-1.5 rounded-full bg-primary" />}
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => changeMode("collapsed")} className="flex items-center justify-between">
                Collapsed
                {mode === "collapsed" && <div className="w-1.5 h-1.5 rounded-full bg-primary" />}
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => changeMode("hover")} className="flex items-center justify-between">
                Expand on hover
                {mode === "hover" && <div className="w-1.5 h-1.5 rounded-full bg-primary" />}
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
